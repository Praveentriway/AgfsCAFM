...
public class CustomerSurveyService {
    ...
    private static final String TAG = CustomerSurveyService.class.getSimpleName();
    ...
    public CustomerSurveyService(Context context, Listener listener) {
        this.mContext = context;
        this.listener = listener;
        mInterface = ApiClient.getClient(mContext).create(ApiInterface.class);
    }

    public CustomerSurveyService(Context context, SaveListener listener) {
        this.mContext = context;
                ...
                new Callback<SurveyLocationResponse>() {
                    ...
                    public void onResponse(
                        ...
                        if (response.isSuccessful()) {
                            ...
                            if (response.body().getStatus().equalsIgnoreCase(ApiConstant.SUCCESS)) {
                                Log.d(TAG, "onResponse success");
                                listener.onReceiveSurveyLocation(
                                        response.body().getObject(), AppUtils.MODE_SERVER
                                );
                            } else
